...
def main():
  ...
  # Video Part end
  # Audio Part start
  queue1_a = Gst.ElementFactory.make('queue')
  pipeline.add(queue1_a)
  ...
  pipeline.add(aacEnc)
  queue2_a = Gst.ElementFactory.make('queue')
  pipeline.add(queue2_a)
  # Audio Part end
  ...
  decodebin.connect("pad-added", decoder_callback, pipeline, queue1, queue1_a)
  ...
  #audio
  queue1_a.link(audioconvert)
  audioconvert.link(resample)
  resample.link(aacEnc)
  aacEnc.link(queue2_a)

  #request audio as well as video pad from muxer
  video_pad= matroskamux.get_request_pad("video_%u")
  queue2.get_static_pad("src").link(video_pad)
  
  audio_pad= matroskamux.get_request_pad("audio_%u")
  queue2_a.get_static_pad("src").link(audio_pad)        
...
def decoder_callback(element, pad, pipeline, video_queue_to_connect, audio_queue_to_connect):
  ...
  if structure_name.startswith("video"):
    ...
    v_queue_pad = video_queue_to_connect.get_static_pad("sink")
    pad.link(v_queue_pad)
    draw_pipeline(pipeline, "udpPipeline_on_video_pad_added")
  elif structure_name.startswith("audio"):
    a_queue_pad = audio_queue_to_connect.get_static_pad("sink")
    pad.link(a_queue_pad)
    draw_pipeline(pipeline, "udpPipeline_on_audio_pad_added")
